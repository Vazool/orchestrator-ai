...
async def process_traveler_notification(traveler, payload, event_id):
    """Handles the full decision funnel for a single traveler in parallel."""
    # 1. Open a fresh Sous Chef connection for this specific task
    ...
    try:
        ...
        if is_opted_in == 0:
            decision, reason, msg = "no_action", "no_consent", None
            # Record the 'No Consent' decision and get out
            ...
            async_db.commit()
            return reason  # We can return early here since no further processing is needed        
        else:
            ...
            if is_dup > 0:
                decision, reason, msg = "no_action", "already_contacted", None
                # Record the 'Duplicate' decision and get out
                async_db.execute(text("""
                    INSERT INTO decisions (event_id, travel_id, decision_type, reason_code)
                    VALUES (:eid, :tid, :dt, :rc)
                """), {"eid": event_id, "tid": traveler.travel_id, "dt": decision, "rc": reason})
                ...
                purpose_feature = purpose_map.get(getattr(traveler, 'travel_purpose', 'leisure').lower(), 0)
                ...
                model_input = pd.DataFrame([{
                    ...
                    "policy_type": traveler.policy_type or 0,
                ...
                if decision != "no_action":
                    ...
                    # THREAD SAFETY: Run the synchronous OpenAI call in a background thread
                    msg = await asyncio.to_thread(generate_ai_message, ai_context)
                else:
                    msg = None

        # STAGE 5: DATABASE RECORDING ---
        # We use 'async_db' exclusively here to avoid cross-contamination
        dec_res = async_db.execute(text("""
            INSERT INTO decisions (event_id, travel_id, decision_type, reason_code)
            VALUES (:eid, :tid, :dt, :rc)
        """), {"eid": event_id, "tid": traveler.travel_id, "dt": decision, "rc": reason})
        ...
        async_db.commit()
        return reason  # The "Manager" uses this to update the dashboard counts

    except Exception as e:
        print(f"❌ Worker Error for {traveler.forename}: {e}")
        async_db.rollback()
        return "error"
    finally:
        async_db.close()
...
async def simulate_event(payload: dict, db: Session = Depends(get_db)):
    ...
    # COMMIT THE EVENT IMMEDIATELY HERE
    db.commit()    
    ...
    sem = asyncio.Semaphore(10) # Limits OpenAI to 10 at a time

    async def throttled_task(traveler):
        async with sem:
            # This calls the helper function we discussed
            return await process_traveler_notification(traveler, payload, event_id)
    ...
    # Launch all tasks in parallel using our new Worker function
    tasks = [throttled_task(t) for t in impacted]
    
    # Wait for all workers to finish. 'results' will be a list of reason_codes.
    results = await asyncio.gather(*tasks)

    # Re-calculate the narrative counts based on what the workers returned
    reason_to_category = {
